...
- **Backend**: Fastify 4, better-sqlite3, Drizzle ORM, hand-rolled OAuth2 (Google/GitHub via `fetch` — **no Passport**), `@fastify/session` + `@fastify/cookie`, `@fastify/websocket`, Vitest (`app.inject()` + real `ws` client).
...
3. **Ownership = 404 missing / 403 not-yours.** A missing resource returns `404 { error: 'Not found' }`; a resource that exists but belongs to another user returns `403 { error: 'Forbidden' }`. Applies to GET/PATCH/DELETE habit, check-ins, and `POST /habits` validation step 2. The WS `ack` still silently ignores unowned/missing habits (no error message).
...
9. **Rate limiting is global**, not auth-only: in-memory sliding window, IP-keyed, 300 req/15s on every route (SPEC §10/§12), registered as the first `onRequest` hook so it runs before session deserialization; `trustProxy: 1` so `request.ip` can't be spoofed via a client-controlled `X-Forwarded-For`.
10. **Name length: server 1–100, client 2–100.** The frontend's stricter lower bound is the UX floor; the server floor stays at 1 so pre-existing 1-char names remain PATCH-able (SPEC §6/§9).
...
- Boot hard-fails (all environments) without a 32+ char `SESSION_SECRET` — no dev fallback (SPEC §3).
